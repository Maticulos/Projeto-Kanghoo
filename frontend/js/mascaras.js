...
function mascaraData(input) {
    ...
    input.addEventListener('input', function(e) {
        ...
        if (isDataNascimento) {
            ...
            // Validar ano se tiver 4 dígitos
            if (valor.length >= 8) {
            ...
            // Validar mês se o ano for o atual
            if (valor.length >= 4) {
                const anoDigitado = parseInt(valor.substring(4, 8) || anoAtual);
                if (anoDigitado === anoAtual) {
                    const mesDigitado = parseInt(valor.substring(2, 4));
                    if (mesDigitado > mesAtual) {
                        // Se o mês for futuro no ano atual, usar o mês atual
                        valor = valor.substring(0, 2) + String(mesAtual).padStart(2, '0') + valor.substring(4);
                        dataFuturaDetectada = true;
                        tipoDataFutura = 'mês';
                    }
                }
            ...
            // Validar dia se o ano e mês forem atuais
            if (valor.length >= 2) {
                const anoDigitado = parseInt(valor.substring(4, 8) || anoAtual);
                const mesDigitado = parseInt(valor.substring(2, 4) || mesAtual);
                if (anoDigitado === anoAtual && mesDigitado === mesAtual) {
                    const diaDigitado = parseInt(valor.substring(0, 2));
                    if (diaDigitado > diaAtual) {
                        // Se o dia for futuro no mês/ano atual, usar o dia atual
                        valor = String(diaAtual).padStart(2, '0') + valor.substring(2);
                        dataFuturaDetectada = true;
                        tipoDataFutura = 'dia';
                    }
                }
            ...
            if (valor.length === 10) {
                ...
                if (partes.length === 3) {
                    ...
                    if (dataDigitada > dataAtual) {
                        ...
                        valor = `${diaAtualStr}/${mesAtualStr}/${anoAtual}`;
                    }
    ...
    if (isDataNascimento) {
        input.addEventListener('keydown', function(e) {
            ...
            // Validar baseado na posição do cursor
            if (posicaoCursor === 0 || posicaoCursor === 1) {
                // Digitando o dia
                if (novoValor.length >= 2) {
                    const diaDigitado = parseInt(novoValor.substring(0, 2));
                    // Se estamos no mês e ano atuais, não permitir dia futuro
                    if (novoValor.length >= 4) {
                        const mesDigitado = parseInt(novoValor.substring(2, 4));
            ...
            } else if (posicaoCursor === 2 || posicaoCursor === 3) {
                // Digitando o mês
                if (novoValor.length >= 4) {
                    const mesDigitado = parseInt(novoValor.substring(2, 4));
                    // Se estamos no ano atual, não permitir mês futuro
                    if (novoValor.length >= 8) {
                        const anoDigitado = parseInt(novoValor.substring(4, 8));
                        if (anoDigitado === anoAtual && mesDigitado > mesAtual) {
                            e.preventDefault();
            ...
            } else if (posicaoCursor >= 4 && posicaoCursor <= 7) {
                // Digitando o ano
                if (novoValor.length >= 8) {
                    const anoDigitado = parseInt(novoValor.substring(4, 8));
                    if (anoDigitado > anoAtual) {
...
async function buscarEnderecoPorCEP(cep, sufixo = '') {
    try {
        // Remove caracteres não numéricos
        ...
        // Faz requisição para a rota do backend
        ...
        mostrarFeedbackCEP(cep, 'error', `Erro ao buscar CEP: ${error.message}`, sufixo);
        return null;
    }
}
    ...
    campo.addEventListener('input', function() {
        ...
        if (cep.length === 8) {
            timeoutId = setTimeout(() => {
                ...
                buscarEnderecoPorCEP(cep, sufixo);
